...
public class MyChannelsAdapter extends RecyclerView.Adapter<MyChannelsAdapter.ViewHolder> {
    ...
    public MyChannelsAdapter(Context context, boolean isDashboard_val, List<ChannelListModel> channelListModels, ChannelListItemClickListener listener, int ScreenWidth, int item_pos, String user_id, String chl_def_img_url_val) {

        this.scrn_context = context.getApplicationContext();
        this.channelListModels = channelListModels;
        ScreenWidth_x = ScreenWidth;
        mListener = listener;
        isDashboard = isDashboard_val;
        userId=user_id;
        itemPos=item_pos;
...
*/
        ...
        holder.channel_layout.setLayoutParams(params);
...
//                    .transform(new CircleTransform(scrn_context))
    ...
    public int getItemCount() {
        if (isDashboard) {
            if (itemPos + 1 < channelListModels.size())
                return 2;
            else
                return 1;
        } else
